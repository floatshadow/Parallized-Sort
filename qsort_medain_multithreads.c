...
void quick_sort(data_type* first,data_type* last){  //sort[begin,end)		
	data_type* cut=__partition(first,last,__medain(*first,*(last-1),*(first+((last-first)>>1))));
...
int main(){
		...
		FILE* fp=freopen("qsort_medain.txt","a+",stdout);
		printf("%.6lf\n",end-start);
		fclose(fp);
		#endif
		#ifdef __CHECK
